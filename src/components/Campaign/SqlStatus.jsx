...
const STATUS_OPTIONS = ["PENDING", "Deal Close", "Completed"]; // ✅ Only these statuses

const SqlStatus = () => {
  const [contacts, setContacts] = useState([]);
  const [loading, setLoading] = useState(false);
  const [comments, setComments] = useState(() => {
    return JSON.parse(localStorage.getItem("sqlContactComments")) || {}; // ✅ Load comments from localStorage
  });
  ...
  // ✅ Fetch contacts with "SQL" status
  const fetchSqlContacts = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${BASE_URL}/getContactsWithSQLStatus`); // ✅ Fetch SQL contacts
      setContacts(response.data);

      // ✅ Ensure comments exist for each contact in localStorage
      const savedComments = JSON.parse(localStorage.getItem("sqlContactComments")) || {};
      ...
      localStorage.setItem("sqlContactComments", JSON.stringify(savedComments));
    } catch (error) {
  ...
  const handleStatusChange = async (contactId, newStatus) => {
    ...
    try {
      ...
      fetchSqlContacts(); // ✅ Refresh the list after update
    } catch (error) {
  ...
  const addComment = (contactId) => {
    ...
    localStorage.setItem("sqlContactComments", JSON.stringify(updatedComments));
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, "SQL_Contacts.xlsx");
  };
